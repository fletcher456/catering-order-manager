...
export class AdaptiveBayesianOptimizer {
  ...
  private defineParameterBounds(): { min: number; max: number }[] {
    return [
      ...
      { min: 0.1, max: 0.5 },    // bilingualConsistency weight (Chinese mode)
      { min: 0.3, max: 0.8 },    // nameToDirectPriceWeight (Chinese mode)
      ...
      { min: 0.3, max: 0.7 },    // pairParsingWeights.nameValidation (Chinese mode)
      { min: 0.3, max: 0.7 },    // pairParsingWeights.priceValidation (Chinese mode)
      { min: 0.1, max: 0.4 },    // pairParsingWeights.bilingualValidation (Chinese mode)
      { min: 0.6, max: 0.9 }     // chineseCharacterDetectionThreshold (Chinese mode)
    ];
  }
  ...
  private arrayToParameters(array: number[]): OptimizationParameters {
    ...
    const phase1: SpatialOptimizationParameters = {
      ...
      confidenceWeights: {
        ...
        bilingualConsistency: array[index++]
      },
      minimumConfidenceThreshold: array[index++],
      nameToDirectPriceWeight: array[index++]
    };
    ...
    const phase3: AssemblyOptimizationParameters = {
      ...
      pairParsingWeights: {
        nameValidation: array[index++],
        priceValidation: array[index++],
        bilingualValidation: array[index++]
      },
      chineseCharacterDetectionThreshold: array[index++]
    };
